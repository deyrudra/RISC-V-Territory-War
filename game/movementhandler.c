...
void initializeCharacter(Character *character, int x, int y,
                         short int *idleCharAsset, short int *walkLeftCharAsset,
                         short int *walkRightCharAsset,
                         short int *jumpLeftCharAsset,
                         short int *jumpRightCharAsset, char team, int id) {
  // Replace all local array declarations with malloc calls
  short int *walkLeftPrevData =
      malloc(sizeof(short int) * LEFTMOVEMENT_WIDTH * LEFTMOVEMENT_HEIGHT);
  short int *walkRightPrevData =
      malloc(sizeof(short int) * RIGHTMOVEMENT_WIDTH * RIGHTMOVEMENT_HEIGHT);
  short int *jumpLeftPrevData =
      malloc(sizeof(short int) * JUMP_WIDTH * JUMP_HEIGHT);
  short int *jumpRightPrevData =
      malloc(sizeof(short int) * JUMP_WIDTH * JUMP_HEIGHT);
  short int *idlePrevData =
      malloc(sizeof(short int) * IDLE_WIDTH * IDLE_HEIGHT);
  short int *leftbootPrev =
      malloc(sizeof(short int) * LEFTBOOT_WIDTH * LEFTBOOT_HEIGHT);
  short int *rightbootPrev =
      malloc(sizeof(short int) * LEFTBOOT_WIDTH * LEFTBOOT_HEIGHT);
  ...
  character->id = id;
  character->withinBlastRadiusBool =
      0;  // 0 means character is not within blast radius
  character->explosionDisplacement =
      0;  // 0 means character is not within blast radius
  ...
  *(character->health) = 100;
  ...
  if (team == 'a') {
    initializeBar(character->healthBar, &redhealthbarpartition,
                  HEALTHBARPARTITION_WIDTH, HEALTHBARPARTITION_HEIGHT,
                  NUM_HEALTH_BAR_PARTITIONS, x + 3, y - 3,
                  NUM_HEALTH_BAR_PARTITIONS);
  } else {
    initializeBar(character->healthBar, &bluehealthbarpartition,
                  HEALTHBARPARTITION_WIDTH, HEALTHBARPARTITION_HEIGHT,
                  NUM_HEALTH_BAR_PARTITIONS, x + 3, y - 3,
                  NUM_HEALTH_BAR_PARTITIONS);
  }
  ...
  character->idleCharacter->velocityX =
      malloc(sizeof(double));  // Allocate memory
  character->idleCharacter->velocityY =
      malloc(sizeof(double));  // Allocate memory
  character->idleCharacter->collidable =
  ...
  character->walkLeftCharacter->velocityX =
      malloc(sizeof(double));  // Allocate memory
  character->walkLeftCharacter->velocityY =
      malloc(sizeof(double));  // Allocate memory
  character->walkLeftCharacter->collidable =
      malloc(sizeof(int));  // Allocate memory
  ...
  character->walkRightCharacter->velocityX =
      malloc(sizeof(double));  // Allocate memory
  character->walkRightCharacter->velocityY =
      malloc(sizeof(double));  // Allocate memory
  character->walkRightCharacter->collidable =
      malloc(sizeof(int));  // Allocate memory

  GameObject *leftJumpingCharacterObj =
  ...
  character->leftJumpingCharacter->velocityX =
      malloc(sizeof(double));  // Allocate memory
  character->leftJumpingCharacter->velocityY =
      malloc(sizeof(double));  // Allocate memory
  character->leftJumpingCharacter->collidable =
      malloc(sizeof(int));  // Allocate memory

  GameObject *rightJumpingCharacterObj =
      (GameObject *)malloc(sizeof(GameObject));
  ...
  character->rightJumpingCharacter->velocityX =
      malloc(sizeof(double));  // Allocate memory
  character->rightJumpingCharacter->velocityY =
      malloc(sizeof(double));  // Allocate memory
  character->rightJumpingCharacter->collidable =
      malloc(sizeof(int));  // Allocate memory
...
void moveCharacter(Character *character, char *direction, int *displacement) {
  if (character == NULL) {
    ...
    // Move left released 1 means released while 0 means it is being pressed
    // (this is for the moving jump where we need sense of both controls at same
    // time)
    if ((strcmp(direction, gameControls[0]) == 0) ||
        (move_left_released == 0)) {  // Checking for move_left
      move_left_released = 0;  // tells us key is still being pressed in case
                               // next control is a jump
      character->state =
          LEFTMOVEMENT;  // update state for drawing the correct asset
      horizontalAcceleration(character, 0);  // update the character's velocity
      *(character->x) =
          *(character->x) +
          *(character->velocityX);  // update position based on new velocity
      *displacement += (int)*(
          character->velocityX);  // update dist travelled for movement limit
      character->lastMovementMade = LEFTMOVEMENT;
    }
    if ((strcmp(direction, gameControls[1]) == 0) ||
        (move_right_released == 0)) {  // Checking for move_right
    ...
    if ((strcmp(direction, gameControls[2]) == 0) &&
        (character->numJumps < 2)) {  // Checking for move_jump
    ...
    if (strcmp(direction, gameControls[3]) ==
        0) {  // Checking for move_left_stop
      *(character->velocityX) = 0;
      move_left_released = 1;
    }
    if (strcmp(direction, gameControls[4]) ==
        0) {  // Checking for move_right_stop
      *(character->velocityX) = 0;
  ...
  if ((((int)*(character->velocityX) == 0) &&
       ((int)*(character->velocityY) == 0))) {
    character->state = IDLE;
  }

  // If character is jumping, as soon as we detect that they are grounded set
  // their y velocity to 0 and update asset
  ...
  // Y-Direction Logic (Gravity Affect on y-velocity, isGrounded poll,
  // y-direction update)
  ...
  if (*(character->x) < SCREEN_WIDTH) {
    character->characterView = LEFTVIEW;
  } else if ((SCREEN_WIDTH <= *(character->x)) &&
...
void checkGrounded(Character *character) {
  ...
  while (boxNode) {
    ...
    // bottom_char, right_char, left_char); printf("Top: %d, Bottom: %d, Right:
    // %d, Left: %d\n", top_plat, bottom_plat, right_plat, left_plat);'

    // Character is between the platform, we check this b/c acceleration logic
    // may skip pixels as it is double math
...
int checkCollision_Characters(Character *a, Character *b) {
  if (!(*a->collidable) || !(*b->collidable))
    return 0;  // Ignore non-collidable objects
...
void updateHealthBar(Character *character) {
  // // This is for change!!! (num_parititons filled will never be >
  // lastRendered since you cant gain health)
  ...
  *(character->healthBar->y) = *(character->y) - 3;
  ...
  for (int i = 0; i < NUM_HEALTH_BAR_PARTITIONS; i++) {
    *(character->healthBar->barObj[i]->x) =
        *(character->healthBar->x) + i * character->healthBar->partitionWidth;
  ...
  for (int i = 0; i < num_partitions_filled; i++) {
    ...
    if (character->state == JUMPINGLEFT ||
        character->prevState == JUMPINGLEFT ||
        character->state == JUMPINGRIGHT ||
        character->prevState == JUMPINGRIGHT) {
    } else {
      renderIn(character->healthBar->barObj[i]);
    }
  }
...
void horizontalAcceleration(Character *character, int directionBool) {
  // if directionBool is equal to 1, then it's positive acceleration, otherwise
  // negative acceleration
  if (directionBool == 1) {
    // velocity is bounded by 1 m/s and 4m/s and we update velocity by adding
    // acceleration*dt
    if (*(character->velocityX) > 3) {
      ...
      *(character->velocityX) =
          *(character->velocityX) + (X_ACCELERATION * DELTATIME);
    }
...
void knockbackCharacter(Character *character, bool characterLeft,
  ...
  if (characterLeft) {
    ...
    *(character->x) =
        *(character->x) +
        *(character->velocityX) * 4;  // update position based on new velocity
    character->explosionDisplacement -=
        *(character->velocityX) * 4;  // subtracting a negative number
  } else {
    horizontalAcceleration(character, 1);
    *(character->x) = *(character->x) + *(character->velocityX) * 4;
    character->explosionDisplacement +=
        *(character->velocityX) * 4;  // adding positive number
  }

  if (grounded == 0) {
    // Update to velocity with constant acceleration
    *(character->velocityY) =
        *(character->velocityY) + (GRAVITY * DELTATIME) * 1.5;
    // Update to position with current velocity
    *(character->y) = *(character->y) + *(character->velocityY);
  }
